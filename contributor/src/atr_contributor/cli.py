"""Run the review tool locally.

  atr-review --reviewer person:slug --reviewer-name "Teacher Name"

Opens a local web page (default http://127.0.0.1:8000) where a teacher reviews the
ingested techniques and writes corrections to data/taxonomy/reviews.json.
"""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="atr-review", description="Teacher review loop")
    p.add_argument("--reviewer", default="person:reviewing-teacher",
                   help="person:<slug> of the reviewing teacher (keyed to the data map)")
    ...
    print(f"ATR review -- reviewer: {who}")
